...
class RealmGenerator:
    ...
    @staticmethod
    def generate_for_player(player: Player, config: AstrBotConfig, config_manager: ConfigManager) -> Optional[
        RealmInstance]:
...
class RealmManager:
    ...
    async def _handle_monster_event(self, p: Player, event: FloorEvent, player_level_index: int) -> Tuple[
        bool, List[str], Player, Dict[str, int]]:
        monster_template_id = event.data["id"]

        if event.type == "boss":
            scaling_factor = self.config["REALM_RULES"].get("REALM_BOSS_SCALING_FACTOR", 1.0)
            enemy = MonsterGenerator.create_boss(monster_template_id, player_level_index, self.config_manager,
