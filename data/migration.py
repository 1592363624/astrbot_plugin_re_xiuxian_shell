...
async def _create_all_tables_v11(conn: aiosqlite.Connection):
    ...
    await conn.execute("""
                       ...
                       """)
    await conn.execute("""
                       CREATE TABLE IF NOT EXISTS player_creation_states
                       (
                           user_id
                           TEXT
                           PRIMARY
                       ...
                       """)
    await conn.execute("""
                       CREATE TABLE IF NOT EXISTS player_creation_data
                       (
                           id
                           INTEGER
                           PRIMARY
...
async def _upgrade_v3_to_v4(conn: aiosqlite.Connection, config_manager: ConfigManager):
    ...
    if 'realm_floor' not in columns: await conn.execute(
...
# 添加新的迁移版本11，用于添加current_map字段
@migration(11)
async def _upgrade_v10_to_v11(conn: aiosqlite.Connection, config_manager: ConfigManager):
    """为players表添加current_map字段"""
    logger.info("开始执行 v10 -> v11 数据库迁移...")
    cursor = await conn.execute("PRAGMA table_info(players)")
    columns = [row['name'] for row in await cursor.fetchall()]
    if 'current_map' not in columns:
        await conn.execute("ALTER TABLE players ADD COLUMN current_map TEXT")
...
async def _upgrade_v11_to_v12(conn: aiosqlite.Connection, config_manager: ConfigManager):
    ...
    if 'avatar' not in columns:
        await conn.execute("ALTER TABLE players ADD COLUMN avatar TEXT")

    # 将player_creation_data中的头像数据迁移到players表
    await conn.execute("""
                       UPDATE players
                       SET avatar = (SELECT data_value
                                     FROM player_creation_data
                                     WHERE player_creation_data.user_id = players.user_id
                                       AND player_creation_data.data_key = 'avatar')
